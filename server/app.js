...
app.get('/book/getSeat/:rc/:rt/:rd/:rm/:ry', async (request, res) => {
	let datas = [request.params.rc, request.params.rt, request.params.rd, request.params.rm, request.params.ry];
...
app.delete('/cart/:uid', async (request, res) => {
	...
	for (let ele of getMy) {
		arr = [];
		arr.push(ele.head_cnt, ele.rs_code, ele.reserve_time, ele.reserve_day, ele.reserve_month, ele.reserve_year);
...
app.post('/pay/orderPayment', async (request, res) => {
	...
	if (data.coupon_code != null && data.coupon_code != undefined) {
		...
		await mysql.query('coupUpdate', [request.body.param.coupon_code, request.body.param.user_id]);
	} else {
		console.log(data);
		result = await mysql.query('orderPayment', data);
		await mysql.query('cartEachDone', request.body.param.reserve_num);
...
app.put('/qnaupdate/:id/:bno', upload.array('files'), async (request, res) => {
	...
	result = await mysql.query('qnaupdate', [qnaInfo, qnaInfo.writer, qnaInfo.qna_code]);
	res.send(result);
});
