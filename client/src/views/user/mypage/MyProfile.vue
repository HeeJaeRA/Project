  ...
  >
    <div style="height: 50%">
      ...
      <img
        ...
        v-if="
          this.userInfoList.profile != null && this.userInfoList.profile != ''
        "
    ...
    <div>
      ...
      <p style="font-weight: bold; margin-top: 10px">
        {{ userInfoList.grade }} {{ userInfoList.user_name }}님
      </p>
      <a style="font-size: 17px; text-decoration-line: none; color: black"
        >팔로워 명 </a
      >/
      <a style="font-size: 17px; text-decoration-line: none; color: black">
        팔로잉 명</a
      ...
      <button class="btn btn-dark rounded-pill px-3" @click="userdelete()">
        회원탈퇴
      </button>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async getUserInfo() {
      ...
      this.userInfoList = (
        await axios.post("/node/getuserinfo", { userId }).catch((err) => {
          console.log(err);
        })
      ).data[0];
