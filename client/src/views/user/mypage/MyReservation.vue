    ...
    <table class="table table-hover" style="text-align: center">
      ...
      <tbody>
        <tr v-if="myReservationList.length == 0">
          <td colspan="10" style="color: gray; text-align: center">
            아직 결제한 내역이 없습니다.
          </td>
        </tr>
        <tr :key="i" v-for="(reserve, i) in myReservationList">
          ...
          <td @click="goToRestaurant(reserve.rs_code)" class="a">
            {{ reserve.rs_name }}
          </td>
          ...
          <td
            v-if="
              reserve.payment_status == '방문확정' &&
              reserve.review_code != null
            "
          >
            <button
  ...
  methods: {
    async userReservationList() {
      ...
      this.myReservationList = (
        await axios
          .get(`/node/reservationList/${userId}/${this.current}`)
          .catch((err) => {
            console.log(err);
          })
      ).data;
