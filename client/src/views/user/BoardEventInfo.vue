<template>
	<div class="container">
		<h4 style="font-family: JalnanGothic; margin-bottom: 40px">EVENT</h4>
		<br />
		...
		<table class="table table">
			<tbody>
				<tr id="eventimg">
					<img :src="`/node/public/uploads/${eventInfo.main_img}`" width="1100px" height="450px" />
				</tr>
				<tr id="eventcontent">
		...
		<div id="btn">
			<button type="button" class="btn btn-outline-secondary" @click="BoardEventList()">목록으로</button>
		</div>
		<button
...
export default {
	...
	methods: {
		async boardEventInfo() {
			let result = await axios.get(`/node/userevent/${this.searchNo}`).catch((err) => console.log(err));
