<template>
    ...
    <p>{{ restList }}</p> -->
		...
		<h1 style="font-weight: bold; text-align: center; color: LightSlateGray; font-family: serif; margin-top: 120px">
			일단테스트중
		</h1>
		<section class="date_time">
			...
			<div class="set">
				<div>
					<p>
						선택한 날짜:
						<span v-if="selectedDate">{{ formatSelectedDate(selectedDate) }}</span>
					</p>
				</div>
				<div class="time">
					<h4>시간을 선택하세요</h4>
					<div class="time_pic2" v-for="(time, i) in timeList" :key="i">
						<label :for="sel" class="time_sel" v-if="selectedDate">
							<input type="radio" v-model="selectTime" :value="time" :id="sel" @change="getSeat" />
				...
				<div class="seat">
					<h4>
						인원 수를 선택해주세요
						<span class="cntCnt">( 예약완료: {{ nowCnt }} / 잔여: {{ restCnt }} )</span>
					</h4>
					<div class="cnt_view">
...
export default {
	...
	methods: {
		...
		async getSeat() {
			let result = await axios
				.get(`/node/book/getSeat/${this.rno}/${this.selectTime.time}/${this.day}/${this.mon}/${this.year}`)
				.catch((err) => console.log(err));
