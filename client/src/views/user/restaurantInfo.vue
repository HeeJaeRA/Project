<template>
  <section class="py-5">
    ...
    <div class="container px-4 px-lg-5 my-5 review-list">
      <h3 style="font-family: JalnanGothic; margin-bottom: 40px">
        Review List
      </h3>

      <p v-if="reviewList.length == 0" class="no-review-message">
        작성된 리뷰가 없습니다.
      </p>
          ...
          <tr v-for="(item, idx) in reviewList" :key="idx">
            ...
            <td>
              <div class="star-rating">
                <div
                  v-for="starClass in getStarClasses(item.star_taste)"
                  :class="starClass"
                ></div>
              </div>
            </td>
            <td>
              <div class="star-rating">
                <div
                  v-for="starClass in getStarClasses(item.star_price)"
                  :class="starClass"
                ></div>
              </div>
            </td>
            <td>
              <div class="star-rating">
                <div
                  v-for="starClass in getStarClasses(item.star_service)"
                  :class="starClass"
                ></div>
              </div>
            </td>
        ...
        <div v-for="rs in restaurants" :key="rs.rs_code" class="col mb-5">
          <div class="card h-100">
            ...
            <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
              <div class="text-center">
                <a
                  class="btn btn-dark text-white mt-auto"
                  @click="moveRsInfo(rs.rs_code)"
                  >상세보기</a
                >
              </div>
            </div>
  ...
  methods: {
    ...
    async openReviewModal(item) {
      ...
      let result = await axios.get(
        `node/adminGetReviewImg/${item.review_code}`
      );
    ...
    async reviewLike(item) {
      try {
        let response = await axios.post(
          `node/rsreviewlike/${item.review_code}`
        );
        console.log(response);
        this.getReviewList();
      } catch (err) {
