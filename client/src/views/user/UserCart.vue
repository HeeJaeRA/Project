<template>
	<div class="container" style="min-height: 800px">
		...
		<div class="delBox">
			<button class="btn btn-outline-dark mt-auto" @click="delCartAll()">예약 전체 비우기</button>
		</div>
		<table class="table table-hover" v-if="this.cartList[0] != null">
			...
			<tbody>
				<tr :key="i" v-for="(reservation, i) in cartList">
					...
					<td>
						<button class="btn btn-secondary" @click="goToEachPay(reservation.reserve_num)">결제</button>
					</td>
					<td>
...
export default {
	...
	methods: {
		...
		async delCartEach(reservation) {
			...
			console.log(obj);
			let result = await axios.put(`/node/cart/${resNo}`, obj).catch((err) => console.log(err));
