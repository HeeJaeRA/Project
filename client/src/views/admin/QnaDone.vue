<template>
	<div>
		...
		<table ref="myDataTable" class="display">
			...
			<tbody>
				<tr v-for="(item, idx) in qnaList" :key="idx" @click="getboard(item.qna_code)">
					<td>{{ idx + 1 }}</td>
					<td>{{ item.title }}</td>
	...
	methods: {
		...
		async getCategory() {
			...
			let result = await axios
				.get(`/node/adminSellerQnaDoneCategory?division=${this.division}&category=${this.category}`)
				.catch((err) => {
					console.log(err);
				});
		...
		//답변완료 전체리스트
		async getQnaList() {
			let result = await axios.get(`/node/adminSellerQnaDone/${this.division}`).catch((err) => {
				console.log(err);
			});
