<template>
	<div class="container">
		<div style="margin-left: 30px; margin-right: 50px; margin-top: 30px">
			<table class="table">
				<h5 style="margin: auto; margin-bottom: 30px; width: 150%">회원 상세 조회</h5>
				<tr>
					<th>아이디</th>
				...
				<tr>
					<th>생년월일</th>
					<td class="text-center">{{ $dateFormat(UserInfo.birthday, 'yyyy-MM-dd') }}</td>
				</tr>
				<tr>
					<th>가입일자</th>
...
export default {
	...
	methods: {
		async getUserInfo() {
			let result = await axios.get(`/node/adminGetUserInfo/${this.searchNo}`).catch((err) => console.log(err));
			console.log(result);
