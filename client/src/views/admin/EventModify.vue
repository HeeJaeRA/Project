<template>
	<div class="container" style="margin: 0 auto">
		<div style="margin-left: 30px; margin-right: 50px; margin-top: 30px">
			<table class="table">
				<h5 style="margin: auto; margin-bottom: 30px; width: 170%">이벤트 정보 수정</h5>
				<tr>
					<th>배너이미지</th>
				...
				<tr>
					<th>할인율</th>
					<td class="text-center"><input type="number" v-model="couponInfo.discount_rate" /> %</td>
				</tr>
		...
		<div>
			...
			<button
				...
				style="margin-left: 0%; padding: 10px; background-color: #ccc; border-color: #ccc"
...
export default {
	...
	methods: {
		async getEventInfo() {
			let result = await axios.get(`/node/adminevent/${this.searchNo}`).catch((err) => console.log(err));
			// console.log(result);
			this.eventInfo = result.data;
			this.couponInfo = result.data;
			this.eventInfo.eventstart_date = this.$dateFormat(this.eventInfo.eventstart_date, 'yyyy-MM-dd');
			this.eventInfo.eventend_date = this.$dateFormat(this.eventInfo.eventend_date, 'yyyy-MM-dd');
			this.couponInfo.start_date = this.$dateFormat(this.couponInfo.start_date, 'yyyy-MM-dd');
			this.couponInfo.end_date = this.$dateFormat(this.couponInfo.end_date, 'yyyy-MM-dd');
