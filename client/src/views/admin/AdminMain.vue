<template>
	<div>
		...
		<div
			...
			"
		>
			<h5 style="font-family: 나눔고딕; margin-bottom: 30px">미답변 QNA (판매자)</h5>
			<table class="table table-hover">
				...
				<tbody>
					<tr v-for="(item, index) in sellerQna" :key="index" @click="getboard(item.qna_code)">
						<td>{{ item.title }}</td>
						<td>{{ item.writer }}</td>
		...
		<div
			...
			"
		>
			<h5 style="font-family: 나눔고딕; margin-bottom: 30px">미답변 QNA (일반회원)</h5>
			<table class="table table-hover">
				<thead>
					<tr>
						<th>제목</th>
				...
				<tbody>
					<tr v-for="(item, index) in userQna" :key="index" @click="getboard(item.qna_code)">
						<td>{{ item.title }}</td>
						<td>{{ item.writer }}</td>
						<td>{{ $dateFormat(item.write_date, 'yyyy-MM-dd') }}</td>
						<td>{{ item.qna_divison }}</td>
...
export default {
	...
	methods: {
		...
		async approve(rscode, e) {
			...
			let result = await axios.put(`/node/adminApprove?status=${this.status}&rscode=${rscode}`);
		...
		async getuserQna() {
			let result = await axios.get(`/node/adminMainQna/일반유저`).catch((err) => {
				console.log(err);
			});
